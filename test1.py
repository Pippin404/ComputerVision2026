...
class SevenSegmentDataset(Dataset):
	"""Dataset that uses the first filename character as the digit label."""
...
def make_data_loaders(
	...
	train_ds = SevenSegmentDataset(train_root, transform=train_tf, max_per_label=50, seed=seed)
...
def parse_args():
	...
	parser.add_argument(
		"--train-dir",
		type=Path,
		default=Path(r"c:\Users\Pippin\Downloads\data\dataset\dataset\train"),
		help="Path to training image root.",
	)
	parser.add_argument(
		"--val-dir",
		type=Path,
		default=Path(r"c:\Users\Pippin\Downloads\data\dataset\dataset\validation"),
